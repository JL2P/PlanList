...
const MyGroupItem = ({ item, onGroupDetail_page }) => {
  return (
    <Grid.Column
      className="recommendGroup_column"
      onClick={() => onGroupDetail_page(item.id)}
    >
      <Link to={`/groupdetail/${item.id}/`}>
        <Card className="group_card" raised>
          <Image
            src={item.galleries[0] ? item.galleries[0].filePath : item.imgUrl}
            className="Group_img"
          />
          <Card.Content>
