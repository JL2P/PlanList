...
const DetailGroupNav = ({
  ...
  const masterAuth =
    member.confirm === "true" && member.manager === "true" ? true : false;
  const memberAuth =
    member.confirm === "true" && member.manager === "false" ? true : false;
  ...
  return (
    <div>
      <Menu pointing secondary>
        ...
        <Menu.Item
          as={Link}
          to={memberConfirm ? `/groupdetail/${group.id}/member` : ""}
          name="멤버"
        ...
        <Menu.Menu position="right">
          {group.master === onLogInUser.accountId ? (
            <Menu.Item
              as={Link}
              to={masterAuth ? `/groupdetail/${group.id}/masterSetting` : ""}
              name="그룹 설정"
            ...
            <Menu.Item
              as={Link}
              to={memberAuth ? `/groupdetail/${group.id}/userSetting` : ""}
              name="그룹 설정"
              active={activeItem === "그룹 설정"}
              onClick={() => onHandleItemClick("그룹 설정")}
              onMouseDown={navAlert}
