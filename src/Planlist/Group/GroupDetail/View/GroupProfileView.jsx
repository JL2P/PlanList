...
const GroupProfilView = ({
  ...
  return (
    <Card style={{ padding: "1rem" }}>
      <Image
        src={groupGallery ? groupGallery : group.imgUrl}
      ...
      <p>
        그룹장 :{" "}
        <Link
          to={`/account/${masterMember.accountId}`}
      ...
      {memberDetail.accountId === onLogInUser.accountId &&
      memberDetail.confirm === "true" ? (
        ""
      ) : memberDetail.accountId === onLogInUser.accountId &&
        memberDetail.confirm === "false" ? (
        <>
          <Button
            inverted
            color="orange"
            onClick={() => onMemberRemove(group.id, memberDetail.id)}
          >
            그룹가입 신청 중입니다.
          </Button>
          <p>
            <small>신청을 취소하시려면 Click</small>
          </p>
        </>
      ) : (
        <Button
