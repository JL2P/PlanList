...
const AuthView = ({
  ...
  const authComponent = authModifymove ? (
    <Form>
      ...
      <Button
        onClick={() => onUserRemove(getAccount.accountId)}
        color="yellow"
        style={{ color: "#000" }}
      >
        삭제
    ...
    <Form>
      ...
      <Button
        onClick={(e) =>
          onUserModify(e, { name, password, birth, accountId, email })
        }
        color="yellow"
        style={{ color: "#000" }}
      >
  ...
  return (
    <div>
      <AuthTemplate name="AuthPage">{authComponent}</AuthTemplate>
    </div>
  );
};
